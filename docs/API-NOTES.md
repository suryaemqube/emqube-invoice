...
| `Common/GetEmployeeList` | POST, body `null` | `IList<EmployeeListModel>` | fields incl. `EmployeeId, FirstName, LastName, Email, DepartmentText, DepartmentValue, UserName, UserType, UserTypeValue, RoleId, RoleName, SiteAccess, IsActive` (boolean), nullable `Password`. |
| `Common/AddUpdateEmployee` | POST, body `EmployeeModel` | `InvoiceMessage` | save payload: `EmployeeId, ProfileId, LoggedInUserId, FirstName, LastName, Email, Phone, Designation, DepartmentValue, UserTypeValue, SiteAccess, UserName, Password, RoleId, IsActive`. `LoggedInUserId` = the logged-in profile's `ProfileId`. |
| `Common/GetRoleList` | POST, body `null` | `IList<RoleModel>` | `RoleId, RoleName` (1=Admin, 2=Manager, 3=Executive, 4=SuperAdmin). |
| `Common/GetParameters` | POST, body `type` (int) | `{ParameterList}` (`RParameterList`) | `5` = invoice types, `6` = discount-rate types (`1`=percent, `2`=flat), `7` = VAT types (`1`=simplified), **`8` = user types**, **`11` = departments** (employee screen), `12` = quote types. |
