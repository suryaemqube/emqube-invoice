...
| `Common/GetSiteuserMenuList` | POST, body `int roleId` | `IList<MenuListModel>` — `{ParentMenu: MenuModel, Childmenu: MenuModel[]}` | `MenuModel`: `MenuItemId, MenuName, MenuParentId, MenuOrder, PageUrl, DepartmentId, RightDelete, RightEdit, RightCreate, RightView`. **No `CssClass`/group field** — see Known Issues. |
